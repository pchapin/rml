...
/**
 * This class illustrates how the SAX might be used. This example is complete enough and yet generic enough
 * to serve as a skeleton for a variety of programs that use SAX. Note that this example does not do a
 * number of things that it might do (like install an error handler). However, I wanted to keep the example
 * small and simple so that the main ideas would not be obscured.
 */
public class Main extends DefaultHandler {
    ...
    private static final String VALIDATION_SCHEMA_ID = "http://apache.org/xml/features/validation/schema";
    ...
    //
    // Public methods. These are callbacks that are invoked by the parser whenever an event occurs.
    //
    ...
    public void startElement(String namespace, String elementName, String rawName, Attributes attrs)
    {
        System.out.println("Entering element: " + elementName + " (" + attrs.getLength() + " attributes)");
        content = null;
    }


    public void endElement(String namespace, String elementName, String rawName)
    {
        if (content != null) {
